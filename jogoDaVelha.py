...
def jogadaEmListaDaMaquina(tabuleiro, lista):
    jogadaPossivel = []
    for l in lista:
        letra, numero = l.split()
        numero = int(numero)-1
        if validaEntradaDisponivel(tabuleiro, letra, numero):
            jogadaPossivel.append(l)

    if len(jogadaPossivel) != 0:
        return random.choice(jogadaPossivel)
    else:
        return None
...
def jogadaMaquina(tabuleiro, jogada):
    ...
    if jogada == 1:
        ...
        letra = ["A", "B", "C"]                  # Definição local das coordenadas
        numero = [0, 1, 2]
       ...
       # teste se é possível o computador vencer na próxima jogada
        ...
        # tenta ocupar um canto, se estiver livre
        canto = jogadaEmListaDaMaquina(tabuleiro, ["A 3", "C 1", "C 3"])
        if canto != None:
            letra, numero = canto.split()
            numero = int(numero)-1
            tabuleiro[letra][numero] = "X"
            return tabuleiro

        else:
            # tenta ocupar um lado, se estiver livre
            lado = jogadaEmListaDaMaquina(tabuleiro, ["A 2", "B 1", "B 3", "C 2"])
            if lado != None:
                letra, numero = lado.split()
                numero = int(numero)-1
                tabuleiro[letra][numero] = "X"
